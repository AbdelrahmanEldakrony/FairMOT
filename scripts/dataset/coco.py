...
class VOCDataset(Dataset):
    ...
    def __init__(self, root='../data/VOCdevkit/VOC2012', resize_size=[512, 512], mode='trainval',
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'trainvalaa':
            img, boxes = self.transform(img, label['boxes'], label['labels'])
            for itr in  range(len(boxes)):
                boxes[itr] = list(boxes[itr])
        ...
        # print('Sampled, classes:',sample, classes)
        # print('Sampled, boxes2:',sample, boxes)
        # print('-'*50)
    ...
    def preprocess_img_boxes(self, image, input_ksize, boxes=None):
        ...
        image_paded = image_resized

        if boxes is None:
            return image_paded
        else:
            boxes[:, [0, 2]] = boxes[:, [0, 2]] * scalew
            boxes[:, [1, 3]] = boxes[:, [1, 3]] * scaleh
    ...
    def collate_fn(self, data):
        ...
        max_h = np.array(h_list).min()
        max_w = np.array(w_list).min()
